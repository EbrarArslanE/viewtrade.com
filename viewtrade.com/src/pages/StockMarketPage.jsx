    ...
    <div className="w-full min-h-screen bg-[#1a1d1e] text-white pb-10 px-8 font-sans">
      ...
      <div className="grid grid-cols-12 gap-6 pt-8 mb-8">
        <div className="col-span-12 md:col-span-4 bg-white/[0.02] border border-white/10 p-6 rounded-[24px] backdrop-blur-md shadow-lg">
           <span className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Piyasa Hacmi (Simüle)</span>
      ...
      <div className="bg-[#1a1d1e] border border-white/5 p-8 mb-8 rounded-3xl shadow-2xl backdrop-blur-xl">
        <div className="flex flex-row flex-wrap gap-6 items-end">
          <div className="flex flex-col gap-2 flex-1">
            ...
            <input
                className="w-full bg-white/5 text-[12px] text-white font-mono px-4 py-3 rounded-xl border border-white/10 outline-none focus:border-emerald-400/40 transition-all shadow-inner"
                type="text"
                placeholder="Sembol ara (Örn: AAPL, TSLA)..."
            ...
            {loading ? (
              <tr>
                <td colSpan="6" className="p-20 text-center">
                   ...
                   <p className="mt-4 font-mono text-[10px] tracking-widest text-gray-500 uppercase">Devler Yükleniyor...</p>
                </td>
              </tr>
            ) : (
              stocks.filter(s => s.symbol.includes(searchTerm)).map((stock) => (
                <tr key={stock.symbol} onClick={() => setSeciliHisse(stock)} className="hover:bg-white/[0.03] transition-all cursor-pointer group">
                  ...
                  <td className="p-6 text-center">
                    <button className="bg-white/5 hover:bg-emerald-400 hover:text-black px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-tighter transition-all"> Detay </button>
                  </td>
                </tr>
              ))
            )}
  ...
  <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in zoom-in duration-200">
    <div className="bg-[#0d0f11] border border-white/10 w-full max-w-2xl rounded-[40px] p-10 relative overflow-hidden shadow-2xl">
      ...
      <div className="mb-10">
        ...
        <h2 className="text-6xl font-black text-white tracking-tighter mt-4 uppercase">{data.name}</h2>
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        ...
        <div className="bg-white/[0.02] border border-white/5 p-6 rounded-3xl">
          <p className="text-[9px] text-gray-600 uppercase font-black tracking-widest">Tahmini Hacim</p>
          <p className="text-lg font-mono font-bold mt-1 text-white/80">${(data.quoteVolume / 1000000).toFixed(2)}M</p>
        </div>
      </div>

      <button className="w-full py-5 bg-emerald-400 hover:bg-emerald-500 text-black font-black uppercase tracking-[0.3em] rounded-2xl transition-all shadow-[0_0_30px_rgba(52,211,153,0.2)]">
         Portföye Ekle
      </button>
    </div>
  </div>
