...
import { HomeIcon, Plus, RefreshCw, Trash2, X } from 'lucide-react'
...
export default function KullaniciListesi({ apiBaseUrl, aktifKullanici, onHome }) {
  ...
  async function handleKullaniciEkle(event) {
    event.preventDefault()
    setHata('')

    // Ekleme yapan kullanıcının şirket ID'sini alıyoruz (Yoksa varsayılan veya hata yönetimi yapılır)
    const sirketId = aktifKullanici?.e_sirket_id || aktifKullanici?.sirket_id || 1;

    // Supabase tablosundaki kolon isimlerine ve e_sirket_id'ye birebir eşliyoruz
    const yeniKullaniciPayload = {
      ...
      e_rol: form.rol.trim(),
      e_durum: form.durum,
      e_sirket_id: sirketId // 👈 İşte burası o hatayı kökten kesip atıyor kanka!
    }
				...
				<div className="modal-backdrop">
					<div className="modal-card modal-wide">
						<div className="modal-head">
							<div>
								<span className="eyebrow">Yeni kullanıcı</span>
						...
						<form className="login-form" onSubmit={handleKullaniciEkle}>
							<div className="profile-form-grid">
								...
								<div className="field-block">
									<label>Rol</label>
									<input name="rol" value={form.rol} onChange={handleFormDegisimi} placeholder="Yetkili / Bayi / Operasyon" required />
								</div>
								<div className="field-block">
									<label>Durum</label>
								...
								<div className="field-block col-md-12">
									...
									<input name="telefon" value={form.telefon} onChange={handleFormDegisimi} placeholder="Telefon Numarası" required />
								</div>
							</div>
